/*
  # Schema Validation and Security Enhancements
...
ALTER TABLE creator_profiles
  ADD CONSTRAINT valid_subscription_price 
    CHECK (subscription_price >= 0),
  ADD CONSTRAINT valid_total_earnings
    CHECK (total_earnings >= 0),
  ADD CONSTRAINT valid_total_subscribers
    CHECK (total_subscribers >= 0);

-- Add validation for content items
...
-- Add validation for scheduled posts
...
-- Add validation for platform connections
ALTER TABLE platform_connections
  ADD CONSTRAINT valid_platform
    CHECK (platform IN ('onlyfans', 'fansly')),
  ADD CONSTRAINT valid_expires_at
    CHECK (expires_at > CURRENT_TIMESTAMP);

-- Add validation for transactions
...
-- Add validation for webhooks
ALTER TABLE webhooks
  ADD CONSTRAINT valid_events
    CHECK (events IS NULL OR array_length(events, 1) > 0);

-- Add validation for api_keys
...
-- Add validation for rate_limits
ALTER TABLE rate_limits
  ADD CONSTRAINT valid_requests_count
